...
//  #TYPE;Table;NomColonne;==Type          :   Applique si la Colonne de la Table est de Type
...
function reload_check_autokey($xaction, $tokenfile, $autokey, $nombre_total, $param_script = '')
	...
	parse_str($_SERVER['QUERY_STRING'], $queries); unset($queries['autokey']); unset($queries['action']);  $param_script = '&' . http_build_query($queries, '', '&amp;');
...
// TRAITEMENT DU REFRESH
	...
	if (! is_numeric($continue)) {
		$installation_erreur = true;
    }
    if ( $installation_stop or $installation_erreur ) $metahead = ''; // INHIBE LE RECHARGEMENT AUTOMATIQUE
...
if ( $installation_erreur )
	{
		echo "ARRET SUR ERREUR ! <br>" . $continue;
		exit;
	}

if ( $installation_impossible )
			...
			if ($line == "" || $line == "\n" || ! $a_traiter || ! in_array( substr($line,0,1) , array('#','@','>')) )
							...
							if ($res = EA_sql_query($sql))
								...
								if (EA_sql_num_rows($res)>0)
									...
									$row = EA_sql_fetch_array($res);
									 //print_r( $row);
									if ( mb_substr($cond[3], 0, 2) == '!=') {
										$cond[3] = mb_substr($cond[3], 2);
									}// SPECIAL EST "!=" on applique si type différent soit le else ci dessous <=> compatibilité avec ancienne fonctionnalité
									if ( mb_substr($cond[3], 0, 2) == '==') { // SPECIAL EST "==" on applique si type est égal
										$cond[3] = mb_substr($cond[3], 2);
				...
				if ($line[0]==">")
					...
					if ($doline==1)  // executer vraiment
						...
						if (mb_strpos($reqmaj, 'ALTER TABLE') !== false) EA_sql_query("Set sql_mode = 'ALLOW_INVALID_DATES';"); // Inhibe sql_erreur sur dates invalides 1292
						if  ($res = EA_sql_query($reqmaj))
							...
							$continue = EA_sql_error().'<br>'.$reqmaj;
							echo '<p>'.$continue.'</p>';
							my_flush(); // Force l'affichage du message
