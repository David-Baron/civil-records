...
if (($xcom == "") or ($xtyp == "")) {
    ...
    while ($ligne = EA_sql_fetch_row($result)) {
        echo '<li><a href="' . $root . '/eclair?xtyp=' . $ligne[0] . '&xcom=' . urlencode($ligne[2]) . '&xdep=' . urlencode($ligne[3]) . '">' . typact_txt($ligne[0]) . ' de ' . $ligne[2] . ' [' . $ligne[3] . ']</a> (' . $ligne[1] . ' actes)</li>';
    }
    echo '</ul>';
} else {
    ...
    if ($cptrow > $config->get('ECLAIR_MAX_ROW')) {
        ...
        while ($ligne = EA_sql_fetch_row($result)) {
            echo '<li>
            <a href="' . $root . '/eclair?xtyp=' . $xtyp . '&xcom=' . urlencode($xcom) . '&xdep=' . urlencode($xdep) . '&xini=' . urlencode($ligne[0]) . '">
