...
if ($xpatr == "" or mb_substr($xpatr, 0, 1) == "_") {
    ...
    <div class="main">
        ...
        liste_patro_1('actes/deces', $path, $xcomm, $xpatr, "Décès / Sépultures", $config->get('EA_DB') . "_dec3", $gid, $note);
    } else {
        ...
        open_page($xcomm . " : Table des décès/sépultures", $root); ?>
            <div class="main">
                ...
                echo '<p>';
                echo 'Commune/Paroisse : <a href="' . $root . '/actes/deces?xcomm=' . $xcomm . '"><b>' . $xcomm . '</b></a>' . geoUrl($gid) . '<br>';
                if ($note <> '') {
                    echo "</p><p>" . $note . "</p><p>";
                }
                ...
                $baselink = $root . '/actes/deces?xcomm=' . $xcomm . '&xpatr=' . $xpatr;
                if ($xord == "N") {
                    $order = $preorder . ", LADATE";
                    $hdate = '<a href="' . $root . '/actes/deces?xcomm=' . $xcomm . '&xpatr=' . $xpatr . 'xord=D">Dates</a>';
                    $baselink = $root . '/actes/deces?xcomm=' . $xcomm . '&xpatr=' . $xpatr . 'xord=N';
                    $hnoms = '<b>' . $nameorder . '</b>';
                } else {
                    $order = "LADATE, " . $preorder;
                    $hnoms = '<a href="' . $root . '/actes/deces?xcomm=' . $xcomm . '&xpatr=' . $xpatr . 'xord=N' . '">' . $nameorder . '</a>';
                    $baselink = $root . '/actes/deces?xcomm=' . $xcomm . '&xpatr=' . $xpatr . 'xord=D';
                    $hdate = '<b>Dates</b>';
                }
                if ($xannee <> "") {
                ...
                if ($nb > 0) {
                    ...
                    while ($ligne = EA_sql_fetch_row($result)) {
                        ...
                        echo '<td>&nbsp;<a href="' . $root . '/actes/deces/acte_details?xid=' . $ligne[3] . '&xct=' . ctrlxid($ligne[0], $ligne[1]) . '&xcomm=' . $xcomm . '&xpatr=' . $xpatr . '">' . $ligne[0] . ' ' . $ligne[1] . '</a></td>';
                        if ($userAuthorizer->isGranted(6)) {
