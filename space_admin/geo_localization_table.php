...
$hcommune = '<a href="' . $root . '/admin/geolocalizations?xord=C' . $url_params . '">Commune</a>';
$hdepart  = '<a href="' . $root . '/admin/geolocalizations?xord=D' . $url_params . '">Département</a>';
$hgeoloc  = '<a href="' . $root . '/admin/geolocalizations?xord=S' . $url_params . '">Géolocalisation</a>';
$baselink = $root . '/admin/geolocalizations?xord=' . $xord . $url_params;
    ...
    <div class="main-col-center text-center">
        ...
        if ($nb > 0) {
            ...
            while ($geoloc = EA_sql_fetch_array($result)) {
                echo '<tr>';
                echo '<td>' . $i . '. </td>';
                echo '<td><a href="' . $root . '/admin/geolocalizations/detail?id=' . $geoloc['ID'] . '">' . ($geoloc['COMMUNE'] ?? '&lt;non précisé&gt;') . '</a></td>';
