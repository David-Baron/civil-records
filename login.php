...
if (!check_version(EA_VERSION, EA_VERSION_PRG) or EA_MAINTENANCE == 1) {
    ...
    $att = "Attention";
    if ($motif == 1) {
        msg('Login ou mot de passe incorrect (vérifiez Majuscules/minuscules) !', $att);
    }
    if ($motif == 2) {
        msg("L'accès à la page que vous voulez consulter est réservé", $att);
    }
    if ($motif == 3) {
        msg('Vos droits sont insuffisants pour accéder à cette page', $att);
    }
    if ($motif == 4) {
        msg("Vous devez vous reconnecter avec le nouveau mot de passe", $att);
    }
    if ($motif == 5) {
