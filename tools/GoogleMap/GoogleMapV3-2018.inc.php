...
class GoogleMapAPI
{
    ...
    function getHeaderJS()
    {
        ...
        $ApiKey = '';
        if (defined('GOOGLE_API_KEY')) $ApiKey = 'key=' . $config->get('GOOGLE_API_KEY') . '&amp;';
        $_headerJS .= "<script type='text/javascript' src='https://maps.google.com/maps/api/js?" . $ApiKey . "sensor=" . (($this->mobile == true) ? "true" : "false") . "'></script>";
    ...
    function getMapJS()
    {
        ...
        $_output .= " * Link https://code.google.com/p/phpgooglemapapiv3/\n";
        ...
        $_output .= " * Originial Link https://www.phpinsider.com/php/code/GoogleMapAPI/\n";
        $_output .= " *************************************************/\n";
    ...
    function geoGetCoords($address, $depth = 0)
    {
        ...
        switch ($this->lookup_service) {
            case 'GOOGLE':
                $ApiKey = '';
                if (defined('GOOGLE_API_KEY')) $ApiKey = 'key=' . $config->get('GOOGLE_API_KEY') . '&amp;';
                $_url = sprintf('https://%s/maps/api/geocode/json?' . $ApiKey . 'sensor=%s&address=%s', $this->lookup_server['GOOGLE'], $this->mobile == true ? "true" : "false", rawurlencode($address));
                $_result = false;
